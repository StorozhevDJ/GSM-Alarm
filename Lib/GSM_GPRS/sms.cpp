...
/*******************************************************************************
Method finds out if there is present SMS in PDU mode with specified status

Note:
if there is new SMS before IsSMSPresent() is executed this SMS has a status 
UNREAD and then after calling IsSMSPresent() method status of SMS is 
automatically changed to READ
...
return: 
        ...
        -----------
        0 - there is no SMS with specified status
        1..20 - position where SMS is stored (suitable for the function GetGSM())
*******************************************************************************/
char SMSGSM::Is_PDU_SMSPresent(byte required_status) 
{
char ret_val = -1;
char *p_char;
byte status;
...
gsm.SimpleWrite("AT+CMGL=");
gsm.SimpleWriteln((int) required_status);
...
/**********************************************************
Method getting PDU SMS from the specified SMS position

position:     SMS position <1..20>

**********************************************************/
/*boolean SMSGSM::Get_PDU_SMS(byte position, SM_PARAM * sms)
{
char smsbuffer[320];

if (gsm.readSMS_PDU(&smsbuffer[0], 320, position))
	{
	return gsmDecodePdu((unsigned char *)smsbuffer, sms);
	}
return false;
}*/

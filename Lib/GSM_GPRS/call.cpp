...
if (status == RX_FINISHED)
	...
	if(gsm.IsStringReceived("+CLCC: 1,1,4,0,0"))
	...
	else if(gsm.IsStringReceived("+CLCC: 1,0,2,0,0"))
		{
		// Исходящий голосовой звонок - идет подключение (набор номера)
		ret_val = CALL_VOICE_CONNECTING;
		}
	else if(gsm.IsStringReceived("+CLCC: 1,0,3,0,0"))
		{
		//Исходящий голосовой звонок - идет дозвон
		ret_val = CALL_VOICE_CONNECTING;
		}
		...
		{
		// other string is not important for us - e.g. GSM module activate call etc.
		// IMPORTANT - each +CLCC:xx response has also at the end string <CR><LF>OK<CR><LF>
		ret_val = CALL_OTHERS;
		}
			...
			if ((first_authorized_pos == 0) && (last_authorized_pos == 0))
				...
				for (i = first_authorized_pos; i <= last_authorized_pos; i++)
					...
					if (gsm.ComparePhoneNumber(i, phone_number))
						{
						// phone numbers are identical authorization is OK
						if (ret_val == CALL_INCOM_VOICE_NOT_AUTH) ret_val = CALL_INCOM_VOICE_AUTH;
						else ret_val = CALL_INCOM_DATA_AUTH;

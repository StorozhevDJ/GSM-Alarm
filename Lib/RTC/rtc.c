...
void RTC_GetTime (RTC_t *rtc)
{
	...
	counter_to_struct( t-946684800, rtc ); // get non DST time
	adjustDST( rtc );
}
...
void RTC_SetTime (const RTC_t *rtc)
{
	...
	my_RTC_SetCounter( cnt+946684800 );
...
if (BKP_ReadBackupRegister(BKP_DR1) != 0xA5A5)
	...
	/* Set initial value */
	RTC_SetCounter(0); // here: 1st January 1970 0:00:00
	/* Wait until last write operation on RTC registers has finished */
	RTC_WaitForLastTask();
...
/*******************************************************************************
* Function Name  : rtc_settime
* Description    : sets HW-RTC with values from UnixTime
* Input          : UnixTime
* Return         : not used
*******************************************************************************/
void RTC_SetUnixTime (long utime)
{
PWR_BackupAccessCmd(ENABLE);
my_RTC_SetCounter( utime );
PWR_BackupAccessCmd(DISABLE);
}
...
{
counter_to_struct(utime-946684800, &*t);
}
...
{
return struct_to_counter(&*t)+946684800;
}

...
//подготовка к приему (выставление таймаута, очистка флагов...)
...
for (i = 0; i < no_of_attempts; i++)
	...
	//Выводим строку с командой в USART
...
{
//Выставляем режим работа с АТ командами
SetCommLineStatus(CLS_ATCMD);
//Устанавливаем скорость работы порта
mySerial.begin(baud_rate);

//Проверяем как отзывается модем (есть ли у него питание), подаем команду "АТ",
//задаем таймаут приема всей посылки 0,5 сек и 0,1 между символами,
//ждем ответа "ОК" с размером в 5 байт
...
if (AT_RESP_ERR_DIF_RESP == SendATCmdWaitResp("AT", 500, 100, "OK", 5))
	...
	for (int i=1;i<7;i++)
		...
		switch (i)
			{
			case 1: mySerial.begin(4800); break;
			case 2: mySerial.begin(9600);	break;
			case 3: mySerial.begin(19200);break;
			case 4: mySerial.begin(38400);break;
			case 5: mySerial.begin(57600);break;
			case 6: mySerial.begin(115200);break;
			// if nothing else matches, do the default
			// default is optional
			}
		...
		if (AT_RESP_OK == SendATCmdWaitResp("AT", 500, 100, "OK", 5)) break;
		}
